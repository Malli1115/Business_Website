...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		// $split_date = $_POST['post_date'];
  //       $orderdate = explode('-',  $split_date);
		// $sdate  = $orderdate[0];
		// $smonth = $orderdate[1];
		// $syear  = $orderdate[2];

		// $post_year_month = $_POST['post_month'].' - '.$syear;

		// If previous image not found and user do not want to change the photo
	    if($previous_photo == '' && $path == '') {
	    	$statement = $pdo->prepare("UPDATE tbl_post SET post_title=?, post_slug=?, post_content=?, post_date=?, category_id=?, meta_title=?, meta_keyword=?, meta_description=?, post_month=?, post_description=? WHERE post_id=?");
	    	$statement->execute(array($_POST['post_title'],$post_slug,$_POST['post_content'],$_POST['post_date'],$_POST['category_id'],'',$_POST['meta_keyword'],'',$_POST['post_month'],$_POST['post_description'],$_REQUEST['id']));
	    }
	    ...
	    if($previous_photo == '' && $path != '') {
	    	...
	    	$statement = $pdo->prepare("UPDATE tbl_post SET post_title=?, post_slug=?, post_content=?, post_date=?, photo=?, category_id=?, meta_title=?, meta_keyword=?, meta_description=?, post_month=?, post_description=? WHERE post_id=?");
	    	$statement->execute(array($_POST['post_title'],$post_slug,$_POST['post_content'],$_POST['post_date'],$final_name,$_POST['category_id'],'',$_POST['meta_keyword'],'',$_POST['post_month'],$_POST['post_description'],$_REQUEST['id']));
	    }
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
		                    <label for="" class="col-sm-2 control-label">Month of post</label>
		                    <div class="col-sm-6">
                                   <input type="text" class="form-control" name="post_month" value="<?php echo $post_month; ?>" readonly>
		                    </div>
		                </div>
						...
						<div class="form-group">
				            <label for="" class="col-sm-2 control-label">Change featured photo (size:1200*800)</label>
				            <div class="col-sm-6" style="padding-top:6px;">
						...
						<div class="form-group">
				         ...
				         <div class="form-group">
							...
							<div class="col-sm-6">
								<textarea class="form-control" name="post_description" style="height:110px;width: 151%" required>
									<?php echo $post_description; ?></textarea>
							</div>
						</div>
				        <div class="form-group">
						...
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Meta keywords (Minimum 5 words) </label>
							<div class="col-sm-9">
